            ...
            <thead class="back">
              <tr>
                ...
                <th scope="col">
                  ...
                  <span style="font-size: x-small">(dd/MM/yyyy)</span>
                </th>
                <th scope="col">
                  Última Fecha de Pago <br />
                  <span style="font-size: x-small">(dd/MM/yyyy)</span>
                </th>
                <th scope="col">
                  Mora <br />
                  <span style="font-size: x-small">COP</span>
                </th>
                <th scope="col">
                  Sub Total <br />
                  <span style="font-size: x-small">COP</span>
              ...
              <tr v-for="fState in getList" v-bind:key="fState.id">
                ...
                <td class="text-center">
                  <section v-if="fState.state.code !== 'PAID'">
                    <a
                      v-if="noosUser?.role?.id === 2 || noosUser?.role?.id === 1"
                      class="edit mx-2"
                      title="Editar Cuenta"
                      @click="openEdit(fState, fState.studentsBill)"
      ...
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <create-update-financial-component
              :financialState="financialData"
...
export default {
  ...
  computed: {
    getList() {
      return this.financialStateList.filter((item) =>
        item.attendant.doc.toLowerCase().includes(this.search.toLowerCase())
      );
    },
    ...mapState(["noosUser"]),
  },
  ...
  methods: {
    async loadData() {
      ...
      FinancialServices.getAll().then((financialList) => {
    ...
    validatefinancialData(fState) {
      ...
      if (
        Date.parse(today) > lastDatePayment &&
        fState.state.code !== CFinancialStates.PAID_STATE.code
      ) {
    ...
    delFinancialState(doc) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          FinancialServices.delete(doc.id).then(() => {
    ...
    async payFinancialState(fState) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          FinancialServices.update({ ...fState, state: CFinancialStates.PAID_STATE }).then(() => {
            Swal.fire({
              title: "Pagado!",
